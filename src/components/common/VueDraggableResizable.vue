...
<script>
import { matchesSelectorToParentElements, addEvent, removeEvent } from '../../utils/draggableResizable/dom.js';
...
export default {
  ...
  methods: {
    ...
    elementMove (e) {
      ...
      const {x,y} = this.$store.state.appScale;
      const tmpDeltaX = (axis && axis !== 'y' ? mouseClickPosition.mouseX - (e.touches ? e.touches[0].pageX : e.pageX) : 0)/x;
      const tmpDeltaY = (axis && axis !== 'x' ? mouseClickPosition.mouseY - (e.touches ? e.touches[0].pageY : e.pageY) : 0)/y;
    ...
    handleMove (e) {
      ...
      const mouseClickPosition = this.mouseClickPosition
      const {x,y} = this.$store.state.appScale;
      const tmpDeltaX = (mouseClickPosition.mouseX - (e.touches ? e.touches[0].pageX : e.pageX))/x
      const tmpDeltaY = (mouseClickPosition.mouseY - (e.touches ? e.touches[0].pageY : e.pageY))/y
